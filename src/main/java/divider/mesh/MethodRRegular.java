...
//here should go triangulation
		...
		divider.figure.Node node1, node2, node3, 
			bl_n=null, br_n=null, tl_n=null, tr_n=null;//bottom&top, left&right;
		divider.figure.Segment left_s=null, right_s=null, top_s=null, bottom_s=null;
		divider.figure.Node left_s_n, right_s_n, bottom_s_n, top_s_n; //begin of the segments 
		int i;
		for(i=0; i<8; i++)//find out relative place of nodes
		{
			node1=(divider.figure.Node)contour.nodes.get( i%4 );
			node2=(divider.figure.Node)contour.nodes.get( (i+1)%4 );
			node3=(divider.figure.Node)contour.nodes.get( (i+2)%4 );
			if( Math.abs(node1.getY()-node2.getY())<Divider.GENERAL_ACCURACY &&
				node3.getY()>node2.getY() )//found bottom left or right node
				{
					...
					bl_n=node2;
					tl_n=node3;	
					tr_n=(divider.figure.Node)contour.nodes.get( (i+3)%4 );
					...
					br_n=node2;
					tr_n=node3;
					tl_n=(divider.figure.Node)contour.nodes.get( (i+3)%4 );
	...
	public String test(Figure figure_) {
		...
		divider.figure.Node node1, node2, node3, node4;
		node1=(divider.figure.Node)contour.nodes.get(0);
		node2=(divider.figure.Node)contour.nodes.get(1);
		node3=(divider.figure.Node)contour.nodes.get(2);
		node4=(divider.figure.Node)contour.nodes.get(3);
		double angle=0.0;
		angle=node1.angle(node4, node2);
		if(Math.abs(angle-Math.PI/2)>Divider.GENERAL_ACCURACY) return error_message;

...
public class Dot {
	...
	/**
	 * Crestes new Dot with coordinates taken from _dot
	 */
	public Dot(Dot _dot) {
		this(_dot.x, _dot.y);
	}
	 ...
	 */
	public static double angle(divider.Dot s1, divider.Dot c, divider.Dot s2) {
	 ...
	 *         -Pi and +Pi
	 */
	public static double signedAngle(divider.Dot s1, divider.Dot c,
			divider.Dot s2) {
...
//		double y1 = c.y - s1.y;
	...
	public static Dot linesIntersection(Dot begin1, Dot end1, Dot begin2,
		...
		d0 = a11 * a22 - a12 * a21;
		if (Math.abs(d0) < Divider.GENERAL_ACCURACY)
			return null;
		else {
			dx = b1 * a22 - a12 * b2;
			dy = a11 * b2 - b1 * a21;
	...
	public static boolean isInsideRectangle(Dot dot, Dot corner1, Dot corner2) {
		...
		if (minX - Divider.GENERAL_ACCURACY < dot.x
				&& dot.x < maxX + Divider.GENERAL_ACCURACY
				&& minY - Divider.GENERAL_ACCURACY < dot.y
				&& dot.y < maxY + Divider.GENERAL_ACCURACY)
			return true;
		else
			return false;
	}

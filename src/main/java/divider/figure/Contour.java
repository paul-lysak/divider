...
public class Contour {
    ...
    public Contour(Figure figure_) 
		...
		public boolean restoreNode(Node node)
		{
			...
			Node nextNode = node.nextSegment.getEnd();
		...
		{
				RectangleArea area = ((Node)nodes.get(0)).nextSegment.calculateBounds();
				Iterator i=nodes.iterator();
				i.next();
				for(; i.hasNext(); )
				{
						area.include( ((Node)i.next()).nextSegment.calculateBounds() );
				}
...
//				System.out.println(nIntersections+"%2="+nIntersections%2);
		...
		public ArrayList nodes = new ArrayList(5);//Do not add items manually to this! use methods of Contour instead
...
//						if(edit())
					if(figure.getCommandStack().doNewCommand(
							new divider.figure.command.EditContourCommand(figure, getThisContour() )))
						{
								figure.panel.redraw();
						}
				}
...
//	public static Color negativeColor = Color.RED;
		...
		public static final double ANGLE_ACCURACY = 1e-30;
		 ...
		 class ContourEditDialog extends divider.AbstractEditingDialog
		{
				ContourEditDialog()
				{

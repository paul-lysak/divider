...
public class MeasureLine {
	...
	public void draw(java.awt.Graphics2D g2) 
	{
		...
		divider.mesh.MeshPanel panel = measure.getMeshPanel();
		g2.setPaint(Measure.measureColor);
		g2.drawLine(panel.xsi(begin.x), panel.ysi(begin.y),
				panel.xsi(end.x), panel.ysi(end.y));
		...
		g2.drawLine(panel.xsi(end.x), panel.ysi(end.y),
				panel.xsi(end.x)-arrow1rel_x, panel.ysi(end.y)-arrow1rel_y);
		g2.drawLine(panel.xsi(end.x), panel.ysi(end.y),
				panel.xsi(end.x)-arrow2rel_x, panel.ysi(end.y)-arrow2rel_y);
		//draw an arrow on begin
		g2.drawLine(panel.xsi(begin.x), panel.ysi(begin.y),
				panel.xsi(begin.x)-arrow1rel_x, panel.ysi(begin.y)-arrow1rel_y);
		g2.drawLine(panel.xsi(begin.x), panel.ysi(begin.y),
				panel.xsi(begin.x)-arrow2rel_x, panel.ysi(begin.y)-arrow2rel_y);
	}
...
//			System.out.println("Element #"+num+"   distance="+selEl.distance);
	...
	public List<Element> getElements()
	{
		...
		{
			element = (Element)allElements.get(i);
			Dot elCenter=element.centralDot();
			Dot.translateCoordinates(begin, end, elCenter);
			w_distance=elCenter.y;
			l_distance=elCenter.x;
			selEl=null;
...
class SelectedElement
{
	...
	SelectedElement(Element el_, double dist_)

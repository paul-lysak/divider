 ...
 * @author gefox
 */
public class MeasureEditor extends fem.divider.AbstractEditingDialog{
		...
		if(line==null)
			{
				line = new MeasureLine(measure, name,
									new fem.geometry.Dot(x1, y1),
									new fem.geometry.Dot(x2, y2));
				line.captureWidth=captureWidth;
			}
			else
			{ 
				line.begin=new fem.geometry.Dot(x1, y1);
				line.end=new fem.geometry.Dot(x2, y2);
				line.name=name;
				line.captureWidth=captureWidth;
			}
	}
